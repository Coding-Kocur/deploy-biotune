...
function renderGrid(filterText = '') {
    ...
    grid.innerHTML = filteredProducts.map((product, index) => {
        ...
        return `
        <div class="bg-[#151515] shadow-lg hover:shadow-2xl p-4 rounded-xl relative group product-card cursor-pointer transition-all duration-300 hover:-translate-y-1" 
             style="animation-delay: ${index * 50}ms"
             onclick="handleCardClick(event, '${product.id}')">
            <div class="aspect-square bg-black/50 rounded-lg mb-4 overflow-hidden border border-white/5">
